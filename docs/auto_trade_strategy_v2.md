...
| 버전 | 날짜 | 주요 변경사항 |
|------|------|---------------|
| v2.2 | 2026-02-05 | 등락률 필터 완화 (-5%~10%), V2>=70 기준 |
| v2.1 | 2026-02-05 | 매수 조건 완화 (Early 80/55, 오전 75/50, 오후 80/55) |
...
│   (완화)      │   (완화)      │   (완화)      │   (완화)      │ 매도   │
│  V2>=80       │   V2>=75      │   V2>=70      │   V2>=80      │ 매수   │
│  V4>=55       │   V4>=50      │   V4>=45      │   V4>=55      │ 금지   │
└─────────────────────────────────────────────────────────────────────┘
```
...
class StrategyConfig:
    ...
    # 매수 임계값 (시간대별) - v2.2 전 시간대 통일
    EARLY_V2_MIN = 70     # (85→70)
    EARLY_V4_MIN = 45     # (60→45)
    MORNING_V2_MIN = 70   # (80→70)
    MORNING_V4_MIN = 45   # (55→45)
    GOLDEN_V2_MIN = 70
    GOLDEN_V4_MIN = 45
    AFTERNOON_V2_MIN = 70 # (85→70)
    AFTERNOON_V4_MIN = 45 # (60→45)

    # 매수 제한 - v2.1 신규
    MAX_BUY_PER_RUN = 5   # 1회 실행당 최대 매수 종목 수
