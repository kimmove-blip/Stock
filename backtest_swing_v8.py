...
V8 Contrarian Bounce (역발상 반등) 전용 백테스트:
...
CHECKPOINT_DIR = OUTPUT_DIR / "backtest_v8"
CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)

from scoring import calculate_score_v8


class SwingBacktestV8:
    """V8 스윙 백테스트 시스템"""

    def __init__(self, max_workers: int = 10, top_n: int = 10,
                 allocation: int = 300_000, min_score: int = 45,
    ...
    def calculate_v7_scores(self, analysis_date: datetime) -> List[Dict]:
        ...
        for code, df in self.stock_data_cache.items():
            try:
                ...
                v7_result = calculate_score_v8(df_filtered)
    ...
    def run_backtest(self, weeks: int = 52, resume: bool = False):
        """백테스트 실행"""
        print("\n" + "=" * 80)
        print("  V8 Contrarian Bounce (역발상 반등) 백테스트 시스템")
    ...
    def save_results(self):
        ...
        excel_path = OUTPUT_DIR / f"backtest_swing_v8_{date_str}.xlsx"
        ...
        # JSON
        json_path = OUTPUT_DIR / f"backtest_swing_v8_{date_str}.json"
        trades = self.results['trades']
        if trades:
            json_data = {
                'run_date': date_str,
    ...
    def print_summary(self):
        """결과 요약 출력"""
        print("\n" + "=" * 80)
        print("  V8 Contrarian Bounce (역발상 반등) 백테스트 결과")
        print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description='V8 스윙 백테스트')
