...
| 시간 | 조건 |
|------|------|
| 09:10 ~ 10:55 | 오전 보수적 (V2≥80, V4≥50) |
| 11:00 ~ 14:29 | 기본 전략 (V2≥75) |
| **14:30 ~ 14:49** | **매수 조건 강화 (+10점)** |
| **14:50 ~** | **신규 매수 금지** |

#### 매도 조건 (트레일링 스탑 + 상한가 보호)

**트레일링 스탑 (config.py)**
```python
TRAILING_STOP_ENABLED = True   # 트레일링 스탑 활성화
TRAILING_STOP_PCT = 5.0        # 고점 대비 -5% 하락 시 익절
TRAILING_STOP_MIN_PROFIT = 5.0 # 최소 +5% 수익 시부터 적용
```

**상한가 보호 (매도 금지)**
```python
STRONG_MOMENTUM_THRESHOLD = 20.0  # +20% 이상 상승 시 매도 금지
UPPER_LIMIT_THRESHOLD = 3.0       # 상한가(+30%)까지 3% 이내 매도 금지
```
...
# V5 >= 70 → 홀딩 유지 (강한 신호)
# V5 < 70 → 정리매도
# 30분 최소보유 조건 미적용 (정리매도 우선)
```

#### 에러 알림
- auto_trader 에러 발생 시 **텔레그램으로 자동 알림**

### 상한가 후보 추적 시스템

> 5분마다 인트라데이 스코어와 함께 상한가 후보 분석 (record_intraday_scores.py)

#### 카테고리별 조건

| 카테고리 | 조건 | 설명 |
|---------|------|------|
| **거래량 폭발** | 거래량 7배+, 등락률 10%+, V2≥60 | 상한가 직전 신호 |
| **강한 모멘텀** | 등락률 15%+ | 당일 강세 |
| **어제 강세** | 어제 +20%+, 오늘 +5%+ | 2일 연속 상승 |
| **대장주-종속주** | 대장주 +5%+, 종속주 갭 2%+ | 테마 캐치업 |

#### 관련 파일

| 파일 | 설명 |
|------|------|
| `record_intraday_scores.py` | 5분마다 스코어 + 상한가 후보 분석 |
| `upper_limit_tracker.py` | 상한가 추적 로직 (대장주-종속주 매핑) |
| `api/routers/upper_limit.py` | API 엔드포인트 |
| `output/upper_limit_candidates.json` | 후보 캐시 파일 |
| `pwa/src/pages/UpperLimitCandidates.jsx` | PWA 화면 |
